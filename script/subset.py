...
def main():
    files = [os.path.abspath(f) for f in subprocess.check_output(['find', OUTPUT_DIR, '-name', '*.html']).decode('utf-8').strip().split('\n')]
    print('Finding subsets of %d fonts on %d pages' % (len(FONTS), len(files)))

    faces_output = subprocess.check_output(['node', './script/faces.js'] + files)
    ...
    for font_and_chars in faces:
        ...
        print('Subset %s from %d to %d bytes (%.2f%% of original size, %d glyphs)' % (
            filename,
            before_size,
            after_size,
